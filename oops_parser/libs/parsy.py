...
class Result(namedtuple('Result', 'status index value furthest expected')):
    ...
    def aggregate(self, other):
        ...
        return Result(
            self.status, self.index, self.value,
            other.furthest, other.expected
        )
...
class Parser(object):
    ...
    def parse_partial(self, string):
        ...
        if not isinstance(string, str):
            raise TypeError('parsy can only parse strings! got {!r}'.format(
                string
            ))
...
line_info = Parser(lambda stream, index:
                   Result.success(index, line_info_at(stream, index)))
